...
// ── 10 · Mini Player → Full Player ──────────────────────────────────────────
// Built with the ibirdui Card, Button and Separator, animated via block-motion.
// The album art is a single persistent element that resizes from a 48px thumbnail
// into a full-width hero (shared-element style, no crossfade); the mini-bar text
// swaps through AnimatePresence and the player's sections stagger in.
const MP_TRACK = {
  title: 'Neon Skyline',
...
// 48px square thumbnail collapsed → full-width hero open, anchored at the p-3 pad.
const MP_ART = 48;
const MP_ART_OPEN_W = 316; // container 340 − 2×12 padding
const MP_ART_OPEN_H = 190;
const MP_PAD = 12;

function MpPlayPauseIcon({ playing }: { playing: boolean }) {
  if (playing) {
    return (
...
/**
 * 10 · Mini Player → Full Player. The collapsed state is a mini player bar
 * (thumbnail + title + artist + a decorative play glyph) that is itself the
 * trigger; on open it morphs into a full player — hero art, title + artist, a
 * labelled progressbar, prev / play-pause / next transport and a shuffle/repeat
 * row. The album art is a single persistent element that resizes from the
 * thumbnail into the hero (no crossfade); the surface springs between the two
 * sizes and the sections stagger in. Play/pause holds its own local state.
 */
function MiniPlayer({ expanded: open, onToggle }: MorphDemoProps) {
  const [playing, setPlaying] = React.useState(true);
  const togglePlay = () => setPlaying((v) => !v);
  const pct = `${MP_TRACK.progress * 100}%`;
    ...
    <MotionProvider>
      ...
      >
        ...
        <AnimatePresence initial={false}>
          {!open && (
            <motion.button
              ...
              type="button"
              onClick={onToggle}
              aria-label={`Open player: ${MP_TRACK.title} by ${MP_TRACK.artist}`}
              aria-expanded={open}
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
              exit={{ opacity: 0 }}
            ...
            >
              ...
              >
                <MpPlayPauseIcon playing={playing} />
              </span>
            </motion.button>
          )}
        </AnimatePresence>
          ...
          {open && (
            ...
            >
              ...
              <motion.div variants={item} className="mt-4 px-1">
                {/* A div, not a heading: it must not force a heading level onto the
                    page (the section already owns the h3). */}
              ...
              <motion.div variants={item} className="mt-4 px-1">
                ...
                <div
                  ...
                  aria-valuemin={0}
                  aria-valuemax={100}
                  aria-valuenow={Math.round(MP_TRACK.progress * 100)}
                  aria-valuetext={`${MP_TRACK.elapsed} of ${MP_TRACK.total}`}
                ...
                <div className="mt-1.5 flex justify-between text-[11px] text-muted-foreground tabular-nums">
                  <span>{MP_TRACK.elapsed}</span>
                  <span>{MP_TRACK.total}</span>
                </div>
              </motion.div>
        ...
        {/* The single, persistent album art — resizes from the 48px thumbnail into
            ...
            (a gradient cover stand-in), never the click target. */}
        <motion.div
          aria-hidden="true"
          ...
          initial={false}
          animate={{
            width: open ? MP_ART_OPEN_W : MP_ART,
            height: open ? MP_ART_OPEN_H : MP_ART,
            borderRadius: open ? 16 : 12,
          }}
          style={{ left: MP_PAD, top: MP_PAD }}
          transition={springs.layout}
        >
          <motion.div
