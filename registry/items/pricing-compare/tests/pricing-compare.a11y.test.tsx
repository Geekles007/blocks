import {
  PricingCompare,
  type PricingComparePlan,
  type PricingCompareRow,
} from '@/components/blocks/pricing-compare';
import { render, screen, within } from '@testing-library/react';
import axe from 'axe-core';
import { describe, expect, it } from 'vitest';

async function expectNoViolations(container: HTMLElement) {
  const results = await axe.run(container, {
    rules: { 'color-contrast': { enabled: false }, region: { enabled: false } },
  });
  expect(results.violations).toEqual([]);
}

const PLANS: PricingComparePlan[] = [
  { name: 'Starter', price: '0 €', period: '/ mois', action: { label: 'Commencer', href: '/s' } },
  {
    name: 'Pro',
    price: '29 €',
    period: '/ mois',
    featured: true,
    action: { label: 'Choisir Pro' },
  },
];

const ROWS: PricingCompareRow[] = [
  { label: 'Projets', values: ['3', 'Illimité'] },
  { label: 'Support prioritaire', values: [false, true] },
];

describe('PricingCompare', () => {
  it('exposes the section title as an h2', () => {
    render(<PricingCompare title="Comparez les offres" plans={PLANS} rows={ROWS} />);
    expect(
      screen.getByRole('heading', { level: 2, name: 'Comparez les offres' }),
    ).toBeInTheDocument();
  });

  it('renders a real table with column and row headers', () => {
    render(<PricingCompare title="Comparez" plans={PLANS} rows={ROWS} />);
    const table = screen.getByRole('table');
    expect(within(table).getByRole('columnheader', { name: /Pro/ })).toBeInTheDocument();
    expect(within(table).getByRole('rowheader', { name: 'Projets' })).toBeInTheDocument();
  });

  it('labels boolean cells for assistive tech', () => {
    render(<PricingCompare title="Comparez" plans={PLANS} rows={ROWS} />);
    expect(screen.getByText('Inclus')).toBeInTheDocument();
    expect(screen.getByText('Non inclus')).toBeInTheDocument();
  });

  it('renders an action with an href as a link and one without as a button', () => {
    render(<PricingCompare title="Comparez" plans={PLANS} rows={ROWS} />);
    expect(screen.getByRole('link', { name: 'Commencer' })).toHaveAttribute('href', '/s');
    expect(screen.getByRole('button', { name: 'Choisir Pro' })).toBeInTheDocument();
  });

  it('has no axe violations in its fullest form', async () => {
    const { container } = render(
      <PricingCompare
        title="Comparez les offres"
        subtitle="Tout, ligne par ligne."
        plans={PLANS}
        rows={ROWS}
      />,
    );
    await expectNoViolations(container);
  });
});
